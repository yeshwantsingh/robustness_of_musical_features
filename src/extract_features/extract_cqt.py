...
datasets_path = '../../data'
datasets = ['homburg', 'carnatic']


def extract_stft(x, sr=22050):
    return np.abs(librosa.cqt(y=x, sr=sr))


def process_song(song):
    sr = 22050
    slices = [song[int(i * sr): int((i + 1) * sr)] for i in np.arange(0, int(len(song) / sr), 3)[:-1]]
