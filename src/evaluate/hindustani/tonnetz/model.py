...
feature, dataset = 'tonnetz', 'hindustani'
...
def pad_image(x, y):
    ...
    x = tf.image.resize_with_pad(x, 10, 44)
    return x, y


def main():
    x_train, x_test, y_train, y_test = prepare_dataset(0.10)
    x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size=0.10, shuffle=True, random_state=42)
    train = tf.data.Dataset.from_tensor_slices((x_train, y_train))
    train = train.shuffle(buffer_size=1000).batch(64).map(pad_image)
    val = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(64).map(pad_image)
    test = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(64).map(pad_image)
    for i in [10]:
        print('\nRunning ', i, ' model')
        try:
            model = build_model(i, (10, 44, 1), 30)
