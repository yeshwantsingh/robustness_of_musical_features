...
feature, dataset = 'stft', 'hindustani'
...
def build_model(number, shape, output):
    if number == 1:
        ...
        # Dense layer
        z = layers.Flatten()(z)
        z = layers.Dense(3096, activation='relu')(z)
...
def main():
    ...
    train = train.shuffle(buffer_size=1000).batch(16)
    val = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(16)
    test = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(16)
    for i in [9]:
        print('\nRunning ', i, ' model')
        try:
            model = build_model(i, (x_train.shape[1], x_train.shape[2]), 30)
