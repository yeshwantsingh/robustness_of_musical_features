...
feature, dataset = 'tonnetz', 'homburg'
...
def main():
    ...
    train = train.shuffle(buffer_size=1000).batch(64)
    val = tf.data.Dataset.from_tensor_slices((x_val, y_val)).batch(64)
    test = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(64)
    for i in [1, 2, 5, 6, 7, 8, 9]:
        print('Running ', i, ' model')
        try:
            #model = build_model(i, (x_train.shape[1], x_train.shape[2]), 9)
            model = build_model(i, (x_train.shape[1], x_train.shape[2]), 9)
