...
feature, dataset = 'spectral_contrast', 'carnatic'
...
def main():
    ...
    trans = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)
   ...
   # test = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(256)
    ...
    for i in [1, 2, 5, 6]:
        ...
        for train_idx, test_idx in trans.split(X, y):
            ...
            history = model.fit(X[train_idx], y[train_idx], epochs=100, batch_size=128,
                            callbacks= [keras.callbacks.ModelCheckpoint('model_' + str(i) + '.h5')]
                            )
            test_acc = model.evaluate(X[test_idx], y[test_idx], return_dict=True)
