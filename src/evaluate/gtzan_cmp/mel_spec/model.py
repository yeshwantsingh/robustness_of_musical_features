...
feature, dataset = 'mel_spec', 'gtzan_cmp'

# setting mixed-precision float16 for faster training on GPU
mixed_precision.set_global_policy('mixed_float16')

def load_data(feature, dataset, frame, slide):
    print(os.getcwd())
    print(feature, dataset)
    features_dir = os.path.join('../', '../', '../', '../', 'features')
    feature_path = os.path.join(features_dir, feature, dataset, '_'+str(frame)+'_'+str(slide))
    print(feature_path)
...
def prepare_dataset(test_size, frame, slide):
    # load data
    data = load_data(feature, dataset, frame, slide)
...
def save_history(number, history, frame, slide):
    with open('history'+str(number)+'_'+str(frame)+'_'+str(slide)+'.json', 'w') as fp:
...
def build_model(number, shape, output):
    if number == 1:
        ...
        z = layers.Reshape((shape[0], shape[1], 1))(inputs)
        effnet = keras.applications.ResNet50V2(weights=None, input_tensor=z, include_top=False)
        z = effnet(z)
        z = layers.GlobalAveragePooling2D()(z)
...
def run(frame, slide):
    x_train, x_test, y_train, y_test = prepare_dataset(0.10, frame, slide)
    ...
    for i in [2, 7]:
        print('Running ', i, ' model, frame: ', frame, ', slide: ', slide)
        model = build_model(i, (x_train.shape[1], x_train.shape[2]), 10)
        ...
        save_history(i, history, frame, slide)
        #plot_model(model, show_shapes=True, to_file='model' + str(i) + '.png')


if __name__ == '__main__':
    #for frame in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
    for frame in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
        for slide in [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
            run(frame, slide)
